...
public class SVMClassifier extends Classifier implements Serializable {
    ...
    private void init()
    {
        ...
        for (int sIndex = 0; sIndex < sLabels.length; sIndex++) {
            ...
            int mIndex = labelAlphabet.lookupIndex(mLabel, false);
    ...
    public Classification classify(ca.uwo.csd.ai.nlp.libsvm.ex.Instance ins)
    {
        KernelManager.setCustomKernel(this.kernel);

        double[] scores = new double[model.nr_class];
        double p = SVMPredictor.predictProbability(ins, model, scores);
        ...
        Instance in = new Instance(ins.getData(),ins.getLabel(),null,null);

        return new Classification(in, this,
                new LabelVector(getLabelAlphabet(), scores));
    }
    /**
