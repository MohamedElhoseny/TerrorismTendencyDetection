...
public class DataMountain extends Display {
    ...
    public static JComponent demo() {
        String datafile = "Models/Evaluation/visualization/demos/data/amazon.txt";
        Table data = null;
        try {
            data = (new DelimitedTextTableReader()).readTable(datafile);
            data.addColumn("image","CONCAT('Models/Evaluation/visualization/demos/data/images/',id,'.01.MZZZZZZZ.jpg')");
        } catch (Exception e) {
            e.printStackTrace();
            System.exit(1);
        }
